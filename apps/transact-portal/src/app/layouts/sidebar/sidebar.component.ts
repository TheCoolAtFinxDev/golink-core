...
export class SidebarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.currentUserName =
      this.accessControl.getCurrentUser()?.name?.trim() || 'Portal User';
    this.menuItems = this.filterMenu(MENU);
  ...
  private filterMenu(items: MenuItem[]): MenuItem[] {
    ...
    for (const item of items) {
      ...
      const filteredItem = this.filterMenuItem(item);
      if (!filteredItem) {
        continue;
      }

      if (pendingTitle) {
        visibleItems.push(pendingTitle);
        pendingTitle = null;
      }
  ...
  private filterMenuItem(item: MenuItem): MenuItem | null {
    ...
    const isVisibleByPermission =
      (!item.permissions || this.accessControl.canAll(item.permissions)) &&
      (!item.permissionsAny || this.accessControl.canAny(item.permissionsAny)) &&
      (!item.moduleKey || this.accessControl.hasModule(item.moduleKey));

    if (!isVisibleByPermission) {
      return null;
    }
